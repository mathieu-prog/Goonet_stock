...
if ($_GET['year']==1) {
	$yearvalue = date("Y")-1;
		...
		$pdf->Cell(180,10,'Rapport des ventes par jour('.$yearvalue.')',0,0,'C');
		$pdf->Ln(10);

		$pdf->setFont('Arial','',10);
		$query0=$pdo->prepare("select DATE_FORMAT(date_sortie, '%d-%m-%Y') date from sorties where date_sortie LIKE ? group by DATE_FORMAT(date_sortie, '%d-%m-%Y') order by date_sortie asc");
